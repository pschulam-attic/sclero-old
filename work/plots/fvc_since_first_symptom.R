...
fvc.data <- merged.train[, c("PtID", "Visit.Date", "perc.FVC.of.predicted", "Date1stSymptom")]
fvc.data <- na.omit(fvc.data)

fvc.data <- transform(fvc.data, year = year_bw_dates(as.Date(Date1stSymptom), as.Date(Visit.Date)))
...
p + labs(title = "% Patients Since First Symptom", x = "At Least X Visits", y = "% Patients")
