...
fvc.data <- merged.train[, c("PtID", "Visit.Date", "perc.FVC.of.predicted", "DateDiagnosis")]
fvc.data <- na.omit(fvc.data)

fvc.data <- transform(fvc.data, year = year_bw_dates(as.Date(DateDiagnosis), as.Date(Visit.Date)))
...
p + labs(title = "% Patients Since Diagnosis", x = "At Least X Visits", y = "% Patients")
