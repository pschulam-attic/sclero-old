...
predict.ridgelm <- function(lm.fit, test) {
  X <- with(test, cbind(1, since.diagnosis, since.diagnosis^2))
  X %*% coef(lm.fit)
}
...
quad_predict <- function(df) {
  ...
  lm.fit <- lm.ridge(FVC.Pre ~ poly(since.diagnosis, degree = 2), data = train, lambda = 10)
